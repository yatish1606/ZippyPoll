...
function ButtonDanger({ title, disabled = false, onClick, iconBefore }: ButtonDangerProps) {
    return (
        <div className={`w-fit text-white flex flex-row items-center justify-center rounded-sm py-2 px-4 ${disabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'} transition-all cursor-pointer`} onClick={onClick}>
